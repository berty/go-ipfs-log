...
func TestLog(t *testing.T) {
	...
	t.Run("sets time.now as id string if id is not passed as an argument", func(t *testing.T) {
		...
		log1, err := ipfslog.NewLog(dag, identities[0], nil)
		require.NoError(t, err)
	...
	t.Run("sets items if given as params", func(t *testing.T) {
		...
		e1, err := entry.CreateEntry(ctx, dag, identities[0], &entry.Entry{Payload: []byte("entryA"), LogID: "A", Clock: entry.NewLamportClock(id1.PublicKey, 0)}, nil)
		require.NoError(t, err)

		e2, err := entry.CreateEntry(ctx, dag, identities[0], &entry.Entry{Payload: []byte("entryB"), LogID: "A", Clock: entry.NewLamportClock(id2.PublicKey, 1)}, nil)
		require.NoError(t, err)

		e3, err := entry.CreateEntry(ctx, dag, identities[0], &entry.Entry{Payload: []byte("entryC"), LogID: "A", Clock: entry.NewLamportClock(id3.PublicKey, 2)}, nil)
		require.NoError(t, err)

		log1, err := ipfslog.NewLog(dag, identities[0], &ipfslog.LogOptions{ID: "A", Entries: entry.NewOrderedMapFromEntries([]iface.IPFSLogEntry{e1, e2, e3})})
		require.NoError(t, err)
	...
	t.Run("sets heads if given as params", func(t *testing.T) {
		e1, err := entry.CreateEntry(ctx, dag, identities[0], &entry.Entry{Payload: []byte("entryA"), LogID: "A"}, nil)
		require.NoError(t, err)

		e2, err := entry.CreateEntry(ctx, dag, identities[0], &entry.Entry{Payload: []byte("entryB"), LogID: "A"}, nil)
		require.NoError(t, err)

		e3, err := entry.CreateEntry(ctx, dag, identities[0], &entry.Entry{Payload: []byte("entryC"), LogID: "A"}, nil)
		require.NoError(t, err)

		log1, err := ipfslog.NewLog(dag, identities[0], &ipfslog.LogOptions{ID: "B", Entries: entry.NewOrderedMapFromEntries([]iface.IPFSLogEntry{e1, e2, e3}), Heads: []iface.IPFSLogEntry{e3}})
		require.NoError(t, err)
	...
	t.Run("returns an error if identity is not net", func(t *testing.T) {
		log1, err := ipfslog.NewLog(dag, nil, nil)
		require.Nil(t, log1)
